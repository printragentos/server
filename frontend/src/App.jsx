/**
 * Printr Agent OS — App.jsx  (v3.1 · PRODUCTION FINAL)
 * Bug fixes:
 *   [FIX 1] POST /agents → (was /agents/create — REST violation)
 *   [FIX 2] POST /pipelines → (was /pipelines/create — REST violation)
 *   [FIX 3] api() error handling surfaced to UI
 *   All other paths unchanged — design system preserved
 */
...
const LC = { info: T.sub, warn: T.yellow, error: T.red, tool: T.amber, tool_result: T.green };
const LP = { info: "   ", warn: "!  ", error: "×  ", tool: "→  ", tool_result: "←  " };
...
const injectGlobal = () => {
  ...
  el.textContent = `
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    body { background: ${T.bg}; color: ${T.text}; font-family: ${T.mono}; -webkit-font-smoothing: antialiased; font-size: 13px; }
    ...
    input:focus, textarea:focus, select:focus { outline: none; border-color: ${T.amber}70 !important; box-shadow: 0 0 0 1px ${T.amber}20; }
...
// ─── API ──────────────────────────────────────────────────────────────────────
// All paths are relative — api() prepends /api so fetch hits /api/<path>
// In dev: Vite proxies /api/* → localhost:3001/api/* (no rewrite)
// In prod: Fastify handles /api/* directly
...
async function api(path, opts = {}) {
  ...
  // Handle non-JSON responses (safety net for misconfigured proxies)
  const contentType = res.headers.get("content-type") ?? "";
  if (!contentType.includes("application/json")) {
    throw new Error(`Server returned non-JSON response (${res.status}). Check API routing configuration.`);
  }
...
function Btn({ children, onClick, variant = "primary", size = "md", disabled, full, style }) {
  ...
  return (
    <button
      ...
      style={{ display: "inline-flex", alignItems: "center", gap: 6, whiteSpace: "nowrap", cursor: disabled ? "not-allowed" : "pointer", opacity: disabled ? 0.35 : 1, transition: "opacity .1s, transform .08s", transform: pressed && !disabled ? "translateY(1px)" : "none", letterSpacing: "0.02em", width: full ? "100%" : "auto", justifyContent: full ? "center" : undefined, ...(variants[variant] ?? variants.ghost), ...(sizes[size] ?? sizes.md), ...style }}
    >
      {children}
    </button>
  );
...
function Input({ value, onChange, placeholder, multiline, rows = 3, disabled, onKeyDown, style, mono = true }) {
  const base = { background: T.card, border: `1px solid ${T.border}`, borderRadius: 2, padding: "8px 11px", color: T.text, fontSize: 12, fontFamily: mono ? T.mono : T.sans, width: "100%", transition: "border-color .12s, box-shadow .12s", resize: multiline ? "vertical" : "none", lineHeight: 1.6, ...style };
  return multiline
...
function Select({ value, onChange, options, disabled, style }) {
  return (
    <select value={value} onChange={e => onChange(e.target.value)} disabled={disabled} style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 2, padding: "8px 11px", color: T.text, fontSize: 12, fontFamily: T.mono, width: "100%", cursor: disabled ? "not-allowed" : "pointer", ...style }}>
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <div style={{ padding: "0 16px", height: 36, borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", background: T.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ display: "flex", gap: 5 }}>{["#3d3d32","#3d3d32","#3d3d32"].map((c, i) => <div key={i} style={{ width: 8, height: 8, borderRadius: "50%", background: c }} />)}</div>
      ...
      <div ref={ref} style={{ flex: 1, overflow: "auto", padding: "14px 0", background: T.bg, position: "relative" }}>
...
function Sidebar({ active, onNav, agentCount, toolCount }) {
  return (
    <aside style={{ width: 200, flexShrink: 0, background: T.surface, borderRight: `1px solid ${T.border}`, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <div style={{ height: 52, borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", padding: "0 18px", gap: 10, flexShrink: 0 }}>
        <div style={{ width: 24, height: 24, background: T.amber, borderRadius: 2, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
          <svg width="12" height="12" viewBox="0 0 12 12" fill="none"><rect x="0" y="0" width="5" height="5" fill="#0c0c0a" /><rect x="7" y="0" width="5" height="5" fill="#0c0c0a" opacity="0.6" /><rect x="0" y="7" width="5" height="5" fill="#0c0c0a" opacity="0.6" /><rect x="7" y="7" width="5" height="5" fill="#0c0c0a" opacity="0.3" /></svg>
        </div>
        <div><div style={{ fontSize: 11, fontWeight: 600, color: T.text, letterSpacing: "0.06em" }}>PRINTR</div><div style={{ fontSize: 9, color: T.sub, letterSpacing: "0.1em" }}>AGENT OS</div></div>
      </div>
      ...
      <nav style={{ flex: 1, padding: "10px 8px", display: "flex", flexDirection: "column", gap: 2 }}>
        {NAV_ITEMS.map(item => {
          ...
          return (
            <button key={item.id} onClick={() => onNav(item.id)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 10px", borderRadius: 2, border: "none", background: isActive ? T.amberGlow : "transparent", color: isActive ? T.amber : T.muted, cursor: "pointer", fontSize: 11, letterSpacing: "0.1em", fontFamily: T.mono, fontWeight: isActive ? 500 : 400, textAlign: "left", width: "100%", transition: "all .12s", borderLeft: isActive ? `2px solid ${T.amber}` : "2px solid transparent" }}>
          ...
          {agents.map(a => (
            <div key={a.id} onClick={() => { setSel(a); setCreating(false); }} style={{ padding: "9px 10px", borderRadius: 2, cursor: "pointer", background: sel?.id === a.id ? T.card : "transparent", borderLeft: sel?.id === a.id ? `2px solid ${T.amber}` : "2px solid transparent", marginBottom: 2, transition: "all .1s" }}>
              ...
              <div style={{ display: "flex", gap: 5 }}>
                <Tag color={a.chain === "solana" ? T.sol : T.evm}>{a.chain.toUpperCase()}</Tag>
                <span style={{ fontSize: 10, color: T.muted }}>{a.tasksRun ?? 0} runs</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ flex: 1, overflow: "auto" }}>
        {creating ? (
          <AgentCreateForm
            ...
            // [FIX 1] POST /agents (was /agents/create — REST-correct)
            onCreate={async (payload) => {
              const data = await api("/agents", { method: "POST", body: payload });
              setCreating(false);
              onRefresh();
              setSel(data.agent); // auto-select new agent
            }}
          />
        ) : selectedAgent ? (
          <AgentDetailPane
...
function AgentCreateForm({ onClose, onCreate }) {
  ...
  return (
    <div className="fade-up" style={{ padding: "24px 28px", maxWidth: 560 }}>
      <div style={{ fontSize: 12, fontWeight: 600, letterSpacing: "0.08em", color: T.text, marginBottom: 20 }}>CREATE AGENT</div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div><Label>SYSTEM PROMPT</Label><Input value={prompt} onChange={setPrompt} placeholder="You are a helpful on-chain agent…" multiline rows={3} /></div>
...
function AgentDetailPane({ agent, onDelete, onRefresh }) {
  ...
  useEffect(() => {
    api(`/agents/${agent.id}/schedule`).then(d => {
      setSched(d.schedule);
      if (d.schedule) { setExp(d.schedule.expression); setSchedIn(d.schedule.input); }
    }).catch(() => {});
  ...
  const saveSchedule = async () => {
    ...
    try {
      const d = await api(`/agents/${agent.id}/schedule`, { method: "POST", body: { expression: exp.trim(), input: schedIn.trim() } });
      setSched(d.schedule);
      ...
      <SectionBlock label="INBOUND WEBHOOK">
        <div style={{ background: T.card, border: `1px solid ${T.border}`, padding: "12px 14px" }}>
          ...
          <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <code style={{ flex: 1, fontSize: 10, color: T.amber, background: T.bg, border: `1px solid ${T.border}`, padding: "6px 10px", letterSpacing: "0.04em", fontFamily: T.mono }}>
...
function PipelinesTab({ agents }) {
  ...
  return (
    <div style={{ display: "flex", height: "100%", overflow: "hidden" }}>
      <div style={{ width: 280, flexShrink: 0, borderRight: `1px solid ${T.border}`, background: T.surface, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ padding: "12px 14px 10px", borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "6px" }}>
          {pipelines.length === 0 && <Empty icon="◈" title="NO PIPELINES" sub="Chain multiple agents into a workflow." action={<Btn size="xs" onClick={() => setCreating(true)}>+ CREATE</Btn>} />}
          {pipelines.map(pl => {
        ...
        {steps.map((step, i) => {
          ...
          return (
            <g key={i}>
              {i > 0 && (
                <g>
                  <line x1={x - ARROW} y1={y + STEP_H / 2} x2={x - 4} y2={y + STEP_H / 2} stroke={STATUS_COLOR[steps[i - 1]?.status] ?? T.border} strokeWidth="1.5" strokeDasharray={step.status === "pending" ? "4 3" : "none"} opacity={step.status === "pending" ? 0.4 : 1} />
                  <polygon points={`${x - 4},${y + STEP_H / 2 - 4} ${x + 2},${y + STEP_H / 2} ${x - 4},${y + STEP_H / 2 + 4}`} fill={STATUS_COLOR[steps[i - 1]?.status] ?? T.border} opacity={step.status === "pending" ? 0.4 : 1} />
                </g>
              )}
              ...
              <text x={x + STEP_W / 2} y={y + STEP_H / 2 + 2} fill={step.status === "pending" ? T.muted : T.text} fontSize={11} fontFamily={T.mono} textAnchor="middle" dominantBaseline="middle" fontWeight={step.status === "running" ? "600" : "400"}>{nm.length < nm.length ? nm + "…" : nm}</text>
...
function CreatePipelineModal({ agents, onClose, onCreate }) {
  ...
  const save = async () => {
    ...
    setBusy(true); setErr(null);
    try {
      // [FIX 2] POST /pipelines (was /pipelines/create — REST-correct)
      await api("/pipelines", {
        method: "POST",
        body: {
          name:  name.trim(),
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(10,10,8,.82)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: 24, backdropFilter: "blur(3px)" }}>
      <div onClick={e => e.stopPropagation()} className="fade-up" style={{ background: T.elevated, border: `1px solid ${T.borderL}`, borderTop: `2px solid ${T.amber}`, width: "100%", maxWidth: 600, maxHeight: "88vh", overflow: "auto", padding: "22px 24px" }}>
        <div style={{ fontSize: 12, fontWeight: 600, letterSpacing: "0.08em", color: T.text, marginBottom: 20 }}>NEW PIPELINE</div>
...
function ToolsTab({ mcp, native, printr, loading, mcpError }) {
  ...
  const filtered = all.filter(t => {
    const mc = filter === "all" || t.chainKey === filter || (filter === "printr" && t.chainKey === "printr");
  ...
  return (
    <div style={{ padding: "22px 26px", overflow: "auto", height: "100%" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <div>
          ...
          <div style={{ fontSize: 11, color: T.sub, marginTop: 3 }}>
            {(mcp ?? []).length} Printr MCP  ·  {(native?.evm ?? []).length} EVM  ·  {(native?.solana ?? []).length} Solana  ·  {(printr ?? []).length} Printr API
          </div>
        </div>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 16, borderBottom: `1px solid ${T.border}`, paddingBottom: 12 }}>
        {filters.map(f => (
          <button key={f.id} onClick={() => setFilter(f.id)} style={{ padding: "5px 12px", border: `1px solid ${filter === f.id ? T.amber + "55" : T.border}`, background: filter === f.id ? T.amberGlow : "transparent", color: filter === f.id ? T.amber : T.muted, fontSize: 10, fontWeight: 500, cursor: "pointer", fontFamily: T.mono, letterSpacing: "0.1em", borderRadius: 1 }}>
            {f.label}
          </button>
        ))}
      </div>

      {mcpError && <div style={{ background: T.yellow + "08", border: `1px solid ${T.yellow}22`, padding: "10px 14px", fontSize: 11, color: T.yellow, marginBottom: 16, lineHeight: 1.7 }}>!  Printr MCP unavailable — {mcpError}<br /><span style={{ color: T.sub }}>Native chain tools and Printr API tools are still active.</span></div>}
      ...
      {!loading && (
        <div style={{ border: `1px solid ${T.border}`, background: T.card, overflow: "hidden" }}>
          ...
          {filtered.map((t, i) => {
            ...
            return (
              <div key={`${t.name}-${i}`}>
                <div onClick={() => setOpen(isOpen ? null : `${t.name}-${i}`)} style={{ display: "grid", gridTemplateColumns: "2fr 4fr 1fr 1fr", gap: 0, padding: "10px 16px", borderBottom: `1px solid ${T.rule}`, cursor: "pointer", background: isOpen ? T.elevated : "transparent", transition: "background .1s" }}>
                      ...
                      {props.map(([k, def]) => {
                        ...
                        return (
                          <div key={k} style={{ display: "flex", gap: 12, alignItems: "flex-start", fontSize: 11 }}>
                            <code style={{ color: T.amber, minWidth: 160, fontFamily: T.mono }}>{k}</code>
  ...
  return (
    <div style={{ height: "100vh", display: "flex", overflow: "hidden", background: T.bg }}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <div style={{ height: 52, borderBottom: `1px solid ${T.border}`, background: T.surface, display: "flex", alignItems: "center", padding: "0 22px", justifyContent: "space-between", flexShrink: 0 }}>
        ...
        <main style={{ flex: 1, overflow: "hidden" }} key={tab}>
          ...
          {tab === "tools"     && <ToolsTab     mcp={tools.mcp} native={tools.native} printr={tools.printr} loading={toolsLoading} mcpError={tools.mcpError} />}
        </main>
      </div>
    </div>
  );
